...
export function DashboardScreen() {
  ...
  const country =
    selectedCountry !== "" ? selectedCountry : (countryKeys[0] ?? "");
  const countryData =
    data && country !== "" ? getCountryData(data, country) : null;
      ...
      loadingComponent={
        <div className="flex items-center justify-center py-24">
          <ThinkingState
            ...
            liveLabel="Fetching regional trends"
          />
        </div>
      }
    >
      <div className="flex flex-col gap-6">
        ...
        >
          <div className="flex flex-col gap-6">
            <TrendingTopics
              topics={(countryData?.trending_topics ?? []) as TrendingTopic[]}
            />
            <ViralHashtags
              hashtags={
            ...
            />
            <ContentGaps
              gaps={countryData?.content_insights?.content_gaps ?? []}
            />
            <CreatorOpportunities
              opportunities={countryData?.creator_opportunities ?? []}
            />
